...
lemma generator_closed: 
  assumes "a \<subseteq> S"
  assumes "a \<in> B"
  shows "a \<in> gen_boolean_algebra S B"
  by (metis Int_absorb2 assms(1) assms(2) gen_boolean_algebra.generator)
...
lemma gen_boolean_algebra_finite:
  assumes "finite Xs"
  shows "finite (gen_boolean_algebra S Xs)"
proof- 
  have 0: "gen_boolean_algebra S Xs = gen_boolean_algebra S (insert S ((\<inter>) S ` Xs))"
    unfolding gen_boolean_algebra_restrict_generators[of S Xs]
    by(rule add_generators, rule gen_boolean_algebra.universe)
  show ?thesis unfolding 0 
  apply(intro fin_gens_imp_fin_algebra)
    using assms apply blast by auto 
qed
...
lemma atoms_in_algebra:
"finite Xs \<Longrightarrow> atoms_of (gen_boolean_algebra S Xs) \<subseteq> (gen_boolean_algebra S Xs)"
  apply(rule subsetI)
  by (meson Set.basic_monos(7) atoms_of_gen_boolean_algebra gen_boolean_algebra.universe 
      gen_boolean_algebra_univ_mono gen_boolean_algebra_finite)

lemma atoms_in_universe:
"finite Xs \<Longrightarrow> a \<in> atoms_of (gen_boolean_algebra S Xs) \<Longrightarrow> a \<subseteq> S"
  using atoms_in_algebra 
  by (metis gen_boolean_algebra_subset subsetD)
...
lemma atom_in_comp:
  ...
  assumes "a \<inter> Y = {}"
  shows "a \<subseteq> S - Y"
proof- 
  have 0: "(S - Y) \<inter> a = {} \<or> a \<subseteq> S - Y"
    by(intro atoms_are_minimal[of a "gen_boolean_algebra S Xs" "S - Y"]
                assms gen_boolean_algebra.complement)
  have 1: "a \<subseteq> S"
    using assms atoms_in_universe 
    by blast
